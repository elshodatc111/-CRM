<div class="table-responsive">
    <table class="table table-hover table-bordered border-primary align-middle" style="font-size: 14px">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ ($childs->currentPage()-1) * $childs->perPage() + $loop->index + 1 }}</td>
                <td class="fw-bold">
                    <a href="{{ route('child_show', $child->id) }}">{{ $child->name }}</a>
                </td>
                <td><span class="badge bg-info text-dark">{{ $child->guvohnoma }}</span></td>
                <td>{{ $child->phone }}</td>
                <td>{{ $child->tkun->format('d.m.Y') }}</td>
                <td class="text-primary fw-bold">{{ $child->formatted_balans }}</td>
                <td>
                    @if($child->is_active)
