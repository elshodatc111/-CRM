  ...
  <section class="section chart_lead">
    <div class="row">
      <div class="col-lg-4">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title text-center">Leadlar (Joriy hafta)</h5>
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
      const funnelOptions = (data, title) => ({
          ...
          xaxis: {categories: ["Lidlar","Jarayonda","Qabul qilindi","Rad etildi"],labels: { show: false },axisBorder: { show: false },axisTicks: { show: false }},
          grid: { show: false },
          legend: { show: false },
          tooltip: {enabled: true,y: {formatter: function(val) { return val + " ta ariza" }}}
      });
      new ApexCharts(document.querySelector("#joriy_hafta"), funnelOptions([10,4,3,2], "Lead")).render();
      new ApexCharts(document.querySelector("#joriy_oy"), funnelOptions([14,8,3,3], "Lead")).render();
      new ApexCharts(document.querySelector("#joriy_yil"), funnelOptions([20,10,4,6], "Lead")).render();
  });
</script>
@endsection